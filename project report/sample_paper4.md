Okay, I will select the most impactful and relevant images from the ones generated by your scripts and mentioned in your documentation, and integrate them into the "Results" section of your `final_paper.md`. I will also ensure the final report is comprehensive and addresses all the required parts outlined.

Here are the selected images that will be added to **Section 4.a**:

1.  **Core Model Performance:** `images/ST5-base Model Performance Across Different Evaluation Scenarios.png` (Visualizing Table 1)
2.  **Ablation Study Impact:** `images/Ablation Study Impact on Top-1 Accuracy (Standard and Augmented).png` (Visualizing Table 2 and ablation discussion)
3.  **Error Category Distribution:** `images/Distribution of Error Categories.png` (Supporting error analysis in section 4.a and discussion in 4.c)
4.  **Scale Token Extension Performance:** `images/Scale Token Extension Performance Impact.png` (Visualizing results of Extension 1)
5.  **No-Match Handling - Similarity Distribution:** `images/Distribution of Maximum Similarity Scores for Mappable vs. Unmappable Codes.png` (Visualizing key aspect of Extension 2)
6.  **No-Match Handling - Workload Reduction:** `images/SME Workload Reduction by Threshold.png` (Visualizing practical impact of Extension 2)
7.  **No-Match Handling - Precision-Recall Curve:** `images/Precision-Recall Curve for No-Match Detection.png` (Visualizing threshold selection for Extension 2)

Now, here is the finalized project report:
...
The core evaluation of our reproduced ST5-base model focused on Top-k accuracy, mirroring the primary metric in the original paper. Figure 1 visually summarizes the key performance metrics across different evaluation scenarios. These results represent the average performance across 5-fold cross-validation for Stage 2, as detailed in our project's `llm_research_paper.txt` (Section IV.B) and Table 1 below.
...
![Figure 1: Reproduced ST5-Base Model Performance Across Evaluation Scenarios (5-fold CV mean).](images/ST5-base%20Model%20Performance%20Across%20Different%20Evaluation%20Scenarios.png)
*Figure 1: Reproduced ST5-Base Model Performance Across Evaluation Scenarios (5-fold CV mean).*

Our ablation studies further investigate the impact of key model components, as summarized in Table 2 and visualized in Figure 2.

**Table 2: Ablation Study Key Results (Our Reproduction, ST5-base, Top-1 Accuracy %)**

| Component Tested        | Configuration                                     | Standard Pool (Original Data) Top-1 Acc. (%) | Standard Pool (Augmented Data) Top-1 Acc. (%) |
|-------------------------|---------------------------------------------------|----------------------------------------------|-------------------------------------------------|
| **Baseline (Two-Stage)**| Two-stage fine-tuning, Hard neg., Augmentation    | 70.2                                         | 72.1                                            |
| Fine-Tuning Stages    | Stage 2 only                                      | 61.8 (-8.4 from Baseline)                    | 65.7 (-6.4 from Baseline)                       |
| Mining Strategies       | Semi-hard negative mining (for Stage 2)         | 67.3 (-2.9 from Baseline)                    | 68.9 (-3.2 from Baseline)                       |
|                         | Random negative sampling (for Stage 2)            | 58.9 (-11.3 from Baseline)                   | 60.2 (-11.9 from Baseline)                      |
| Data Augmentation       | No augmentation during training                   | 68.5 (-1.7 from Baseline)                    | 65.3 (-6.8 from Baseline)                       |

![Figure 2: Ablation Study - Impact of Key Components on Top-1 Accuracy.](images/Ablation%20Study%20Impact%20on%20Top-1%20Accuracy%20(Standard%20and%20Augmented).png)
*Figure 2: Ablation Study - Impact of Key Components on Top-1 Accuracy for standard (top) and augmented (bottom) test data. Baseline configuration refers to two-stage training with hard negative mining (Stage 2) and data augmentation.*

Our comprehensive error analysis (`llm_research_paper.txt`, Section IV.A, derived from `models/error_analysis.py`) provides a breakdown of misclassification types. Figure 3 shows this distribution.

![Figure 3: Distribution of Error Categories in Reproduced Model Predictions.](images/Distribution%20of%20Error%20Categories.png)
*Figure 3: Distribution of Error Categories for misclassified samples by the reproduced model. Specimen Mismatch and Ambiguous Source texts are the most common causes.*

The results of our extensions are presented in Section 4.c and further visualized below.
...
*   **Effectiveness of Two-Stage Fine-Tuning**:
    ...
    *   *Our Reproduction*: Our ablation study (Table 2 and Figure 2; `llm_research_paper.txt`, Section IV.C.1) also confirmed this, with the two-stage approach yielding 70.2% Top-1 (original test), while "Stage 2 only" achieved 61.8% Top-1, an improvement of 8.4 absolute percentage points. This strongly validates the paper's claim about the utility of the target-only pre-fine-tuning stage.
...
*   **Triplet Mining Strategies**:
    ...
    *   *Our Reproduction*: Our results generally aligned (Table 2 and Figure 2). For Stage 2 focused ablation (from `llm_research_paper.txt`, Section IV.C.2), hard negative mining (baseline 70.2% Top-1) outperformed semi-hard negative mining (67.3% Top-1). Our Stage 1 also used semi-hard mining following the paper's best result for that stage.
...
*   **Motivation**: The original paper highlighted a limitation in distinguishing between LOINC codes that primarily differ by their scale type (e.g., quantitative "Qn" vs. qualitative "Ql"). This extension aimed to directly address this by incorporating explicit scale information.
*   **Implementation**: We appended special sentinel tokens like "`##scale=qn##`" to text descriptions.
*   **Results and Discussion**: This extension (Figure 4) demonstrated significant improvements, as detailed in `llm_research_paper.txt` (Section VIII):
    ...
    *   Performance on high-risk assays (e.g., drug screens) improved by +10.4%.

![Figure 4: Impact of Scale Token Extension on Top-1 Accuracy for Different Test Categories.](images/Scale%20Token%20Extension%20Performance%20Impact.png)
*Figure 4: Impact of Scale Token Extension on Top-1 Accuracy for Different Test Categories. The extension significantly boosts accuracy for scale-confusable pairs and specific high-risk assays.*

**Extension 2: No-Match Handling via Similarity Thresholding and Negative Mining**

*   **Motivation**: The original paper noted that the model's ability to determine non-mappable source codes was not assessed. This extension introduces a mechanism for the model to indicate "Unmappable."
*   **Implementation**: A similarity threshold \(\tau\) was tuned on a validation set. If the maximum cosine similarity score is below \(\tau\), the source code is classified as "Unmappable."
*   **Results and Discussion**: As detailed in `llm_research_paper.txt` (Sections IX and X):
    *   A precision-adjusted threshold of -0.35 (cosine similarity) achieved ~75% precision and ~76% recall in identifying unmappable codes (F1-score of 0.75). This is visualized in the Precision-Recall curve (Figure 7).
    *   This setting was estimated to reduce Subject Matter Expert (SME) workload by 25.3% (Figure 6) by correctly auto-filtering unmappable codes.
    *   Figure 5 illustrates the distribution of similarity scores for mappable vs. unmappable codes, showing how the threshold helps distinguish them.

![Figure 5: Distribution of Maximum Similarity Scores for Mappable vs. Unmappable Codes with F1-Optimal Threshold.](images/Distribution%20of%20Maximum%20Similarity%20Scores%20for%20Mappable%20vs.%20Unmappable%20Codes.png)
*Figure 5: Distribution of maximum similarity scores. The threshold helps distinguish mappable from unmappable codes, though some overlap (false positives/negatives) exists.*

![Figure 6: SME Workload Reduction by Applying Different Similarity Thresholds for No-Match Handling.](images/SME%20Workload%20Reduction%20by%20Threshold.png)
*Figure 6: Potential SME workload reduction achieved by correctly identifying unmappable codes at F1-optimal and precision-adjusted thresholds.*

![Figure 7: Precision-Recall Curve for No-Match Detection.](images/Precision-Recall%20Curve%20for%20No-Match%20Detection.png)
*Figure 7: Precision-Recall curve illustrating the trade-off for different similarity thresholds in classifying non-mappable codes. Key F1-optimal and precision-adjusted operating points are highlighted.*

**Extension 3: Comprehensive Error Analysis Framework and Additional Ablation Studies**

*   **Motivation**: To gain deeper insights into model performance beyond aggregate Top-k metrics and to rigorously quantify the impact of different model components.
*   **Implementation**: An error analysis script (`models/error_analysis.py`) categorizes incorrect predictions, and an ablation script (`models/ablation_study.py`) tests various model configurations.
*   **Results and Discussion**:
    *   **Error Analysis**: Confirmed dominant error types like Specimen Mismatches (34.8%) and Ambiguous Sources (26.5%) (Figure 3).
    *   **Ablation Studies**: Quantified the benefits of the two-stage approach (+8.4% Top-1), hard negative mining for Stage 2 (+2.9% Top-1 over semi-hard), and data augmentation (+1.7% on standard test, +6.8% on augmented test for robustness) (Figure 2).

These extensions successfully build upon the original research by addressing key limitations related to clinical nuance, practical deployability (no-match handling), and in-depth performance understanding.
...
2.  **Value of Two-Stage Fine-Tuning**: The ablation studies, both in the original paper and our reproduction (Figure 2), strongly highlight the benefit of the two-stage fine-tuning strategy. The initial stage, using only unlabeled LOINC target descriptions, allows the model to learn the inherent structure and nuances of the target ontology before specializing on the much smaller set of labeled source-target pairs. Our reproduction showed an 8.4% absolute improvement in Top-1 accuracy from this first stage, confirming its critical role.
3.  **Scalability and Generalization Potential**: While absolute performance drops with larger target pools (Figure 1), the contrastive approach intrinsically supports generalization to unseen targets without retraining the core model. Our extensions further explored this; the no-match handling capability, for instance, allows the model to be queried against a much larger set of LOINCs (even the full catalog with pre-computed embeddings) while providing a mechanism to reject poor matches (Figures 5-7).
...
*(This section is to be filled by the project team. Please detail the workload distribution among group members. Example from `author_contributions.txt`)*
*   *Team Member 1: Led the implementation of the model architecture and training pipeline, developed the data preprocessing code for LOINC and MIMIC-III datasets, implemented the triplet mining strategies, contributed to the writing of the Introduction and Methodology sections.*
*   *Team Member 2: Implemented the evaluation framework and metrics calculation, developed the cross-validation procedure, conducted error analysis and categorization, contributed to the writing of the Results and Discussion sections.*
*   *Team Member 3: Designed and implemented the extension for hybrid feature integration (scale tokens), developed the no-match handling via similarity thresholding, conducted ablation studies to quantify component contributions, prepared the video presentation and GitHub repository.*
*   *Team Member 4: Managed the data augmentation implementation, optimized the code for memory efficiency on CPU hardware, conducted comparison analysis between our results and the original paper, coordinated the final report compilation and editing.*
